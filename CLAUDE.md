...
- **LLM**: any OpenAI-compatible chat completions endpoint. Ollama is the default (`http://localhost:11434/v1` for local dev on Apple Silicon — Metal GPU; `http://ollama:11434/v1` inside the compose network for server). Anthropic / OpenAI / OpenRouter all work by swapping `LLM_BASE_URL` + `LLM_API_KEY` + `LLM_MODEL`.
