...
# DB-only mirror profile (run FIRST — `with` resets the config): postgres +
# import/replication tooling, no web UI, no Solr.
sudo admin/configure with alt-db-only-mirror

# Publish Postgres to the host. Default is 5432, which clashes with
# rose-cinema's own PG — override to 5433 via a compose override in local/
# after adding the option:
sudo admin/configure add publishing-db-port
# then in local/compose/publishing-db-port.yml (or a new local override),
# change the mapping to "5433:5432" and set a real postgres password.
...
# Replication (free MetaBrainz token: https://metabrainz.org/supporters/account-type;
# stored in local/secrets/metabrainz_access_token). The cron applies packets
...
- Replication applies packets daily at 03:00 UTC by default; if the mirror
  falls behind or the annual MusicBrainz schema upgrade lands, follow
  musicbrainz-docker's upgrade notes. rose-cinema tolerates a down/broken
  mirror (logs loudly, skips deep cuts).
