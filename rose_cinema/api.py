...
async def update_dj(
    ...
    if body.voice_ticks is not None:
        existing.voice_ticks = body.voice_ticks
    ...
    return ExportData(
        djs=[
            DJExport(
                ...
                tts_voice_ref=d.tts_voice_ref, voice_ticks=d.voice_ticks,
            )
            for d in all_djs
        ],
        stations=[
...
async def import_all(body: ExportData, session: AsyncSession = Depends(get_session)):
    ...
    for dj_data in body.djs:
        ...
        record = await dj_repo.create(DJRecord(
            ...
            tts_voice_ref=dj_data.tts_voice_ref, voice_ticks=dj_data.voice_ticks,
        ))
